class Person < ApplicationRecord
  ...
  scope :corbett, -> { where("housing_facilities.name LIKE 'Corbett%' OR
                             ...
                             conferences.name like 'Colorado Operations Summer Mission'").joins(:conferences).joins(:housing_facilities) }

  scope :durrell, -> { where("(housing_facilities.name LIKE 'Durward%' OR
                             ...
                             conferences.name not like 'Colorado Operations Summer Mission'").joins(:conferences).joins(:housing_facilities) }

  scope :rams_horn, -> { where("housing_facilities.name LIKE 'Summit%' OR
                             housing_facilities.name LIKE 'Academic Village'").joins(:housing_facilities) }

  scope :meal_adult, -> { where("date_part('year', age( '2017-07-01',  birthdate)) > 10") }
  scope :meal_child, -> { where("date_part('year', age( '2017-07-01',  birthdate)) > 4 and date_part('year', age( '2017-07-01',  birthdate)) <= 10") }
