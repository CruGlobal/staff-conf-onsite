...
class JuniorSenior::SumChildCostTest < ServiceTestCase
  ...
  test 'junior/senior 4 weeks' do
    @child.update!(childcare_weeks: [4], childcare_deposit: false)
    @service.call
    assert_equal Money.new(512_00), @service.charges[:junior_senior]
  end

  test 'junior/senior 4 weeks + deposit' do
    @child.update!(childcare_weeks: [4], childcare_deposit: true)
    @service.call
    assert_equal Money.new(544_00), @service.charges[:junior_senior]
  end
end
